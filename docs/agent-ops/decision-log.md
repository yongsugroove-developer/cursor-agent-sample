...
| 2026-02-24 | DEC-009 | Cursor subagent structure hardening | "Research current trends in detail and add the best form, not the minimum." | Added root `AGENTS.md`, modular `.cursor/rules/*.mdc`, and focused `.cursor/agents/*.md` files so Cursor can persist project instructions and delegate FE/BE/QA/security work with clearer boundaries. | In Progress |
| 2026-02-24 | DEC-010 | Subagent model tuning | "Recommend per-agent model allocation and apply the change." | Kept implementation workers on inherited main-model quality, kept QA verifier on fast model, and raised `security-reviewer` from `fast` to `inherit` because this project's SMTP/API/secrets review benefits more from stronger reasoning than raw speed. | In Progress |
